...
if( !defined('YS_CFDBH_PLUGIN_VERSION') ) : //Plugin Version
	define('YS_CFDBH_PLUGIN_VERSION',	'1.4.0');
endif;
...
function ys_cfdbh_init(){
    if( is_admin() ){
        ...
        if( isset($_REQUEST['ys_cfdbh_csv']) &&  sanitize_text_field( $_REQUEST['ys_cfdbh_csv'] ) == true  && isset( $_REQUEST['ys_cfdbh_nonce'] ) ) {
            ...
            if ( ! wp_verify_nonce( $nonce, 'ys_cfdbh_nonce') ) {
...
function  ys_cfdbh_show_admin_page(){ 
  ...
  /*/ Check IF contact form 7 plug in is activate or not /*/
	...
	wp_enqueue_style( 'ys-cfdbh-style', plugin_dir_url( __FILE__ ) . 'css/ys-cfdbh-style.css', array(), YS_CFDBH_PLUGIN_VERSION );
...
function ys_cfdbh_update_view_list() {
  /*/ check request /*/
  	if( isset( $_POST['conatctformId'] ) && isset( $_POST['Datalist'] ) && isset( $_POST['conatctformId'] ) ) :
		...
		if( update_post_meta( $conatctformId, 'show_form_columns', maybe_serialize($columns) ) ) :  
